...
class SweepDirt(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.act_type in ['robot_joint_pose']:
            self.num_obs_dict = {
                "full_state": 18*2 + 13 + 1,
            }
        else:
            self.num_obs_dict = {
                "full_state": 6*2 + 13 + 1 + 1 + 1,
            }
        ...
        if self.repre_type not in ['handcrafted', 'eureka']:
            goal_hand = world2screen(torch.tensor([[0, 0.22, 0.45], [0, -0.11, 0.4]]).unsqueeze(0).to(self.device), 
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name, object_asset_file in object_asset_files.items():
            ...
            if object_asset_name in ["dustpan", "broom", "dirt1", "dirt2"]:
                object_asset_options.fix_base_link = False
            else:
                object_asset_options.fix_base_link = True
            object_asset_options.disable_gravity = False
            object_asset_options.linear_damping = 50
            object_asset_options.angular_damping = 50
        ...
        for object_asset_name in ["dustpan", "broom", "broom_holder", "dirt1", "dirt2", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.5, -0.1, 0.5)
            self.camera_lookat_pos = gymapi.Vec3(0, 0, 0.2)
        elif self.camera == 'up':
            self.camera_place_pos = gymapi.Vec3(0.5, 0.0, 0.5)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.2)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'broom':
                    self.broom_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 0)
                elif object_asset_name == 'dustpan':
                    self.pan_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 0)
                elif object_asset_name == 'dirt1':
                    self.dirt1_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 0)
                elif object_asset_name == 'dirt2':
                    self.dirt2_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 0)
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            if self.eureka_seed == 0:
                dist = torch.norm(self.broom_pose[:,:3].to(self.device) - self.pan_pose[:,:3].to(self.device), dim=-1)
                reward = -dist
                temperature = torch.tensor(10.0)
                reward = torch.tanh(reward / temperature)
            elif self.eureka_seed == 1:
                distance_pan_dirt = torch.norm(self.pan_pose - self.dirt1_pose, dim=-1) + torch.norm(self.pan_pose - self.dirt2_pose, dim=-1)
                distance_broom_dirt = torch.norm(self.broom_pose - self.dirt1_pose, dim=-1) + torch.norm(self.broom_pose - self.dirt2_pose, dim=-1)
                temp_distance = torch.tensor(0.01, device=self.broom_pose.device)
                reward_distance = torch.exp(-temp_distance * distance_pan_dirt)
                temp_broom_dirt = torch.tensor(0.01, device=self.broom_pose.device)
                reward_broom_dirt = torch.exp(-temp_broom_dirt * distance_broom_dirt)
                reward = reward_distance + reward_broom_dirt
            elif self.eureka_seed == 2:
                alpha: torch.Tensor = torch.tensor(0.1)
                temperature_a: torch.Tensor = torch.tensor(0.1)
                distance_to_goal: torch.Tensor = torch.sqrt(((self.broom_pose[:,:3].to(self.device) - torch.tensor([0, 0.13, 0],device=self.device))**2).sum(dim=-1))
                clipped_distance: torch.Tensor = torch.clamp(distance_to_goal, 0.0, 100.0)
                reward_reaching_goal: torch.Tensor = - alpha * clipped_distance
                reward: torch.Tensor = temperature_a * torch.exp(reward_reaching_goal / temperature_a)
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError

        goal_broom_pose = torch.tensor([0, 0.13, 0, 0, 0, 0, 1], device=self.device)
        goal_dirt1_pose = torch.tensor([-0.05, -0.05, 0.03, 0, 0, 0, 1], device=self.device)
        goal_dirt2_pose = torch.tensor([0.05, -0.05, 0.03, 0, 0, 0, 1], device=self.device)
        goal_pan_pose = torch.tensor([0, -0.15, 0, 0, 0, 0, 1], device=self.device)
        broom_pose = self.broom_pose.clone()
        dirt1_pose = self.dirt1_pose.clone()
        dirt2_pose = self.dirt2_pose.clone()
        pan_pose = self.pan_pose.clone()
        achived_goal = torch.norm(broom_pose - goal_broom_pose, dim=1) < 0.05
        ...
        self.scores = 1 - torch.norm(broom_pose[:, :3] - goal_broom_pose[:3], dim=1) / 0.18
        ...
        if self.repre_type == 'handcrafted':
            reward = (4 - torch.norm(broom_pose-goal_broom_pose, dim=1)/0.21 - torch.norm(dirt1_pose-goal_dirt1_pose, dim=1)/0.15 - torch.norm(dirt2_pose-goal_dirt2_pose, dim=1)/0.2 - torch.norm(pan_pose-goal_pan_pose, dim=1)/0.05) / 4
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        ...
        self.extras['indicator'] = broom_pose.clone()[:, 1].mean().unsqueeze(0)  # only y axis, from 0.34 to -0.01
    ...
    def compute_observations(self):
        ...
        self.broom_pose = self.rigid_body_states[:, self.broom_body_handle, 0:7]
        self.broom_linvel = self.rigid_body_states[:, self.broom_body_handle, 7:10]
        self.broom_angvel = self.rigid_body_states[:, self.broom_body_handle, 10:13]
        self.pan_pose = self.rigid_body_states[:, self.pan_body_handle, 0:7]
        self.dirt1_pose = self.rigid_body_states[:, self.dirt1_body_handle, 0:7]
        self.dirt2_pose = self.rigid_body_states[:, self.dirt2_body_handle, 0:7]
    ...
    def compute_full_state(self):
        ...
        self.obs_buf[:, 2*self.num_dofs : 2*self.num_dofs + 7] = self.broom_pose  # 7
        self.obs_buf[:, 2*self.num_dofs + 7 : 2*self.num_dofs + 10] = self.broom_linvel  # 3
        self.obs_buf[:, 2*self.num_dofs + 10 : 2*self.num_dofs + 13] = self.broom_angvel  # 3
    ...
    def _setup_attachable_body(self, ):
        ...
        self.body_tms = []
        self.related_tms = []
        for object_name in ['broom', 'dustpan']:
            ## load attach info
            ...
            if object_name == "broom":
                att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'broom_body')  # broom body
            else:
                att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'dustpan_body')  # dustpan body
            for att in object_attachs:
                ...
                att['rotation_matrix'] = att['rotation_matrix'] @ R.from_euler('xyz', [1.57, 1.57, 1.57]).as_matrix()
                att_tm = np.eye(4)
                att_tm[:3, :3] = att_rot
