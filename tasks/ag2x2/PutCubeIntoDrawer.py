...
class PutCubeIntoDrawer(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.act_type in ['robot_joint_pose']:
            self.num_obs_dict = {
                "full_state": 21*2 + 13 + 1,
            }
        else:
            self.num_obs_dict = {
                "full_state": 9*2 + 7 + 3 + 3 + 1 + 1 + 1,
            }
        ...
        if self.viewer != None:
            cam_pos = gymapi.Vec3(0.8, 0.0, 1.0)
        ...
        if self.repre_type not in ['handcrafted', 'eureka']:
            goal_hand = world2screen(torch.tensor([[0.1, 0, 0.374], [0.265, 0, 0.405]]).unsqueeze(0).to(self.device), 
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name, object_asset_file in object_asset_files.items():
            ...
            object_asset_options.linear_damping = 15
            object_asset_options.angular_damping = 15
            if object_asset_name == 'cube':
                object_asset_options.fix_base_link = False
                object_asset_options.linear_damping = 20
                object_asset_options.angular_damping = 20
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(-0.2, 0.5, 0)  # right
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., -0.5]).as_quat())
            robot_another_start_pose = gymapi.Transform()
            robot_another_start_pose.p = gymapi.Vec3(0.7, -0.5, 0)  # left
            robot_another_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 0.5]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(-0.2, 0, 0.6)  # right
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 0.]).as_quat())
            robot_another_start_pose = gymapi.Transform()
            robot_another_start_pose.p = gymapi.Vec3(0.05, 0, 0.45)  # left
        ...
        for object_asset_name in ["drawer", "cube", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.3, 0, 0.8)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.3)
        elif self.camera == 'up':
            self.camera_place_pos = gymapi.Vec3(0.2, 0, 0.9)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.3)
        elif self.camera == 'down':
            self.camera_place_pos = gymapi.Vec3(0.4, 0, 0.7)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.3)
            ...
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.7)
        else:
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'drawer':
                    self.drawer_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'drawer_joint_top')
                if object_asset_name == 'cube':
                    self.cube_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 0)
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            if self.eureka_seed == 0:
                dist_reward_coefficient: torch.Tensor = torch.tensor(1.0)
                vel_reward_coefficient: torch.Tensor = torch.tensor(0.05)
                dist_drawer_cube: torch.Tensor = torch.norm(torch.tensor([0,0,0.375],device=self.device) - self.cube_pose[..., :3], dim=-1)
                linear_velocity: torch.Tensor = torch.norm(self.cube_linvel, dim=-1)
                angular_velocity: torch.Tensor = torch.norm(self.cube_angvel, dim=-1)
                dist_reward: torch.Tensor = -dist_drawer_cube * dist_reward_coefficient.to(self.device)
                vel_reward: torch.Tensor = -(linear_velocity + angular_velocity) * vel_reward_coefficient.to(self.device)
                reward: torch.Tensor = dist_reward + vel_reward
            elif self.eureka_seed == 1:
                goal_distance = torch.norm(torch.tensor([0,0,0.375],device=self.device) - self.cube_pose[..., 0:3], dim=-1)
                dist_reward = -torch.tanh(0.1 * goal_distance)
                linvel_norm = torch.norm(self.cube_linvel, dim=-1)
                velocity_punish = -torch.tanh(0.1 * linvel_norm)
                reward = dist_reward + velocity_punish
            elif self.eureka_seed == 2:
                temp1 = 0.1
                temp2 = 0.01
                cube_drawer_dist = torch.norm(self.cube_pose[:, :3] - torch.tensor([0,0,0.375],device=self.device), dim=-1)                    
                dist_reward = torch.exp(-temp1 * cube_drawer_dist)
                cube_drawer_quat_diff = self.cube_pose[:, 3:] - torch.tensor([0,0,0,1],device=self.device)
                cube_drawer_rot_dist = torch.norm(cube_drawer_quat_diff, dim=-1)                        
                rot_reward = torch.exp(-temp2 * cube_drawer_rot_dist)
                reward = dist_reward + rot_reward
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError

        goal_drawer_dof_pos = 0.3
        drawer_dof_pos = self.drawer_dof_pos.clone()
        achived_pull_goal = torch.abs(goal_drawer_dof_pos - drawer_dof_pos) < 0.05
        goal_cube_pos = torch.tensor([0.15, 0, 0.374], device=self.device)
        cube_pos = self.cube_pose.clone()[:, :3]
        achived_put_goal = torch.norm(cube_pos - goal_cube_pos, dim=1) < 0.15
        achived_goal = achived_pull_goal & achived_put_goal
        ...
        self.scores = 0.5 * drawer_dof_pos / goal_drawer_dof_pos + 0.5 * (1 - torch.norm(cube_pos - goal_cube_pos, dim=1) / 0.35)
        ...
        self.extras['indicator_pull'] = drawer_dof_pos.clone().mean().unsqueeze(0)  # from 0 to 0.3
        self.extras['indicator_x'] = cube_pos.clone()[:,0].mean().unsqueeze(0)
        self.extras['indicator_y'] = cube_pos.clone()[:,1].mean().unsqueeze(0)
        self.extras['indicator_z'] = cube_pos.clone()[:,2].mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.robot_another_dof_pos = self.robot_another_dof_state[..., 0]
        self.robot_another_dof_vel = self.robot_another_dof_state[..., 1]
        self.drawer_dof_pos = self.dof_states[:, self.drawer_dof_handle, 0]
        ...
        self.cube_pose = self.rigid_body_states[:, self.cube_body_handle, 0:7]
        self.cube_linvel = self.rigid_body_states[:, self.cube_body_handle, 7:10]
        self.cube_angvel = self.rigid_body_states[:, self.cube_body_handle, 10:13]
    ...
    def compute_full_state(self):
        ...
        self.obs_buf[:, 2*self.num_dofs : 2*self.num_dofs + 7] = self.cube_pose  # 7
        self.obs_buf[:, 2*self.num_dofs + 7 : 2*self.num_dofs + 10] = self.cube_linvel  # 3
        self.obs_buf[:, 2*self.num_dofs + 10 : 2*self.num_dofs + 13] = self.cube_angvel  # 3
    ...
    def pre_physics_step(self, actions):
        ...
        if self.act_type in ['robot_joint_pose']:
            ...
            dof_states[:, self.robot_another_dof_indices[[2]], 0] = 3.0
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['drawer']:
            ...
            for att in object_attachs:
                ...
                att['translation'] += [0, 0.1, 0]
                if att_point[2] > 0.4:
                    att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'drawer_top')
                elif att_point[2] < 0.3 and att_point[2] > 0.2:
                    att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'drawer_middle')
                else:
                    att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'drawer_bottom')
                pos_new = np.array(att['rotation_matrix'])
                ...
                att['rotation_matrix'] = att['rotation_matrix'] @ R.from_euler('xyz', [1.57, 0, -1.57]).as_matrix()
                att_tm = np.eye(4)
                att_tm[:3, :3] = att_rot
        ...
        for object_name in ['cube1']:
            ...
            for att in object_attachs:
                ...
                att_point = object_rot @ np.array(att_point) + object_trans
                att_rot = object_rot @ (np.array(att_rot) @ R.from_euler('xyz', [0., 0., 0]).as_matrix())  # till here, attach pose in world
                att_tm = np.eye(4)
                att_tm[:3, :3] = att_rot
                att_tm[:3, 3] = att_point
                self.att_tms.append(att_tm)
                att_rot = R.from_matrix(att_rot).as_quat()
                self._attachable_points.append(att_point)
                att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'cube')
                self._attachable_handles.append(att_rb_handle)
                body_tm = np.eye(4)  # initial attached rigid part pose matrix in world
                body_tm[:3, 3] = self.rigid_body_states[0, att_rb_handle, :3].cpu().numpy()
                body_tm[:3, :3] = R.from_quat(self.rigid_body_states[0, att_rb_handle, 3:7].cpu().numpy()).as_matrix()
