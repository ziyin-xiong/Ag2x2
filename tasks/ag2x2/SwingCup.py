...
class SwingCup(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.viewer != None:
            cam_pos = gymapi.Vec3(0.4, 0.0, 0.3)
        ...
        if self.repre_type not in ['handcrafted', 'eureka']:
            goal_hand = world2screen(torch.tensor([[0.0215, 0.0854, 0.1121], [-0.0215, -0.0854, 0.1121]]).unsqueeze(0).to(self.device), 
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name in ["cup", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.35, 0.0, 0.2)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.08)
        elif self.camera == 'up':
            self.camera_place_pos = gymapi.Vec3(0.35, 0.0, 0.25)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.08)
        elif self.camera == 'down':
            self.camera_place_pos = gymapi.Vec3(0.35, 0.0, 0.15)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.08)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'cup':
                    self.cup_base_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 0)  # cup base
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            if self.eureka_seed == 0:
                cup_diff = self.cup_pose - torch.tensor([0, 0, 0.0863, 0, 0, 0.70683, 0.70739], device=self.device)
                reward = torch.exp(-torch.sum(cup_diff[..., 3:]**2, dim=1) / (2 * 1**2))
            elif self.eureka_seed == 1:
                cup_rot_z = torch.acos(self.cup_pose[:, 6]) * 2 * torch.sign(self.cup_pose[:, 3])
                cup_rot_z = torch.where(cup_rot_z < 0, cup_rot_z + 2 * np.pi, cup_rot_z)
                rotate_reward = torch.exp(-((cup_rot_z - np.pi / 2) ** 2) / (2 * np.pi / 8))
                reward = rotate_reward
            elif self.eureka_seed == 2:
                goal = torch.tensor([0,0,0.7,0.7],device=self.device)
                dot_product = torch.matmul(self.cup_pose[:,3:], goal)
                dot_product = torch.clip(dot_product, -1, 1)
                angle = 2*torch.arccos(torch.abs(dot_product))
                reward = -torch.exp(angle)
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError

        goal_cup_pose1 = torch.tensor([0, 0, 0.0863, 0, 0, 0.70683, 0.70739], device=self.device)  # 前面的handle转到右侧,后面的handle转到左侧
        goal_cup_pose2 = torch.tensor([0, 0, 0.0863, 0, 0, -0.70683,  0.70739], device=self.device)
        cup_pose = self.cup_pose.clone()  # cup base link pose
        achived_goal1 = torch.norm(cup_pose - goal_cup_pose1, dim=1) < 0.05
        achived_goal2 = torch.norm(cup_pose - goal_cup_pose2, dim=1) < 0.05
        achived_goal = torch.logical_or(achived_goal1, achived_goal2)  # 正转反转都可以
        ...
        self.scores1 = 1 - torch.norm(cup_pose - goal_cup_pose1, dim=1) / 0.77 # from 0,0,0,1 to 0,0,0.7,0.7
        self.scores2 = 1 - torch.norm(cup_pose - goal_cup_pose2, dim=1) / 0.77 # from 0,0,0,1 to 0,0,-0.7,0.7
        self.scores = torch.max(self.scores1, self.scores2)  # 正转反转都可以
    ...
    def compute_observations(self):
        ...
        self.cup_pose = self.rigid_body_states[:, self.cup_base_handle, 0:7]
        self.cup_linvel = self.rigid_body_states[:, self.cup_base_handle, 7:10]
        self.cup_angvel = self.rigid_body_states[:, self.cup_base_handle, 10:13]
    ...
    def compute_full_state(self):
        ...
        self.obs_buf[:, 2*self.num_dofs : 2*self.num_dofs + 7] = self.cup_pose  # 7
        self.obs_buf[:, 2*self.num_dofs + 7 : 2*self.num_dofs + 10] = self.cup_linvel  # 3
        self.obs_buf[:, 2*self.num_dofs + 10 : 2*self.num_dofs + 13] = self.cup_angvel  # 3
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['cup']:
            ...
            for att in object_attachs:
                ...
                att_rot = object_rot @ (att['rotation_matrix'] @ R.from_euler('xyz', [0, 0, 1.57]).as_matrix())
                if att_point[0] > 0.:
                    att['translation'] += [0.1, 0, 0]
                    att['rotation_matrix'] = np.array([[-1,0,0],[0,-1,0],[0,0,1]]) @ R.from_euler('xyz', [0, 1.57, 0]).as_matrix()
                    att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'link_1')  # right and front handle with body
                else:
                    att['translation'] += [-0.1, 0, 0]
                    att['rotation_matrix'] = att['rotation_matrix'] @ R.from_euler('xyz', [0, 1.57, 0]).as_matrix()
                    att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'link_2')  # left and back handle
