...
class StraightenRope(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.act_type in ['robot_joint_pose']:
            self.num_obs_dict = {
                "full_state": 33*2 + 13*2 + 1,
            }
        else:
            self.num_obs_dict = {
                "full_state": 21*2 + 13*2 + 1 + 1 + 1,
            }
        ...
        if self.viewer != None:
            ...
            cam_target = gymapi.Vec3(0.0, 0.0, 0)
        ...
        if self.repre_type not in ['handcrafted', 'eureka']:
            goal_hand = world2screen(torch.tensor([[0, 0.35, 0.02], [0, -0.4, 0.02]]).unsqueeze(0).to(self.device), 
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name in ["rope", "mark1", "mark2", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.1, 0.0, 0.5)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0)
        elif self.camera == 'up':
            self.camera_place_pos = gymapi.Vec3(0.05, 0.0, 0.6)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0)
        elif self.camera == 'down':
            self.camera_place_pos = gymapi.Vec3(0.2, 0.0, 0.4)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'rope':
                    self.rope_head_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 0)
                    self.rope_tail_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 15)

                if object_asset_name == 'rope':
                    dof_states = self.gym.get_actor_dof_states(env_ptr, object_handle, gymapi.STATE_ALL)
                    dof_states['pos'] = np.array([0.1,0.3,0.5,0.8,0.5,0.3,0.2,0.1,-0.1,-0.3,-0.5,-0.8,-0.5,-0.3,-0.1])
                    dof_states['vel'] = 0.0
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            if self.eureka_seed == 0:
                align_reward_temperature = .1
                distance_between_rope_ends = torch.linalg.norm(self.ropehead_pose[..., :2] - self.ropetail_pose[..., :2], dim=-1)
                align_reward = torch.exp(-distance_between_rope_ends / align_reward_temperature)
                altitude_reward_temperature = .1
                height_difference = self.ropehead_pose[..., 2] - self.ropetail_pose[..., 2]
                altitude_reward = torch.exp(height_difference / altitude_reward_temperature)
                reward = align_reward + altitude_reward
            elif self.eureka_seed == 1:
                rope_displacement = self.ropehead_pose - self.ropetail_pose
                reward_straight = -torch.norm(rope_displacement)
                reward_ropehead_vel = -torch.norm(self.ropehead_linvel)
                reward_ropetail_vel = -torch.norm(self.ropetail_linvel)
                reward = reward_straight + reward_ropehead_vel + reward_ropetail_vel
            elif self.eureka_seed == 2:
                rope_orientation = torch.atan2(self.ropehead_pose[..., 2] - self.ropetail_pose[..., 2], self.ropehead_pose[..., 0] - self.ropetail_pose[..., 0])
                target_orientation = torch.zeros_like(rope_orientation,device=self.device)
                orientation_dist = torch.sub(rope_orientation, target_orientation)
                directional_reward = torch.exp(-1. * orientation_dist)
                height_dist = torch.abs(self.ropehead_pose[..., 1]) + torch.abs(self.ropetail_pose[..., 1])
                height_reward = torch.exp(-1. * height_dist)
                reward = directional_reward * height_reward
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError

        goal_ropehead_pose = torch.tensor([0, 0.35, 0.02, 0.5, 0.5, -0.5, 0.5], device=self.device)
        goal_ropetail_pose = torch.tensor([0, -0.4, 0.02, 0.5, 0.5, -0.5, 0.5], device=self.device)
        ropehead_pose = self.ropehead_pose.clone()
        ropetail_pose = self.ropetail_pose.clone()
        achived_head = torch.norm(ropehead_pose - goal_ropehead_pose, dim=1) < 0.1
        achived_tail = torch.norm(ropetail_pose - goal_ropetail_pose, dim=1) < 0.1
        achived_goal = achived_head & achived_tail
        ...
        self.scores = 0.5*(1 - torch.norm(ropehead_pose[:,:3] - goal_ropehead_pose[:3], dim=1) / 0.15) + 0.5*(1 - torch.norm(ropetail_pose[:,:3] - goal_ropetail_pose[:3], dim=1) / 0.15)
        ...
        self.extras['indicator_hx'] = ropehead_pose.clone()[:, 0].mean().unsqueeze(0)
        self.extras['indicator_hy'] = ropehead_pose.clone()[:, 1].mean().unsqueeze(0)
        self.extras['indicator_tx'] = ropetail_pose.clone()[:, 0].mean().unsqueeze(0)
        self.extras['indicator_ty'] = ropetail_pose.clone()[:, 1].mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.ropehead_pose = self.rigid_body_states[:, self.rope_head_handle, 0:7]
        self.ropehead_linvel = self.rigid_body_states[:, self.rope_head_handle, 7:10]
        self.ropehead_angvel = self.rigid_body_states[:, self.rope_head_handle, 10:13]
        self.ropetail_pose = self.rigid_body_states[:, self.rope_tail_handle, 0:7]
        self.ropetail_linvel = self.rigid_body_states[:, self.rope_tail_handle, 7:10]
        self.ropetail_angvel = self.rigid_body_states[:, self.rope_tail_handle, 10:13]
    ...
    def compute_full_state(self):
        ...
        self.obs_buf[:, 2*self.num_dofs : 2*self.num_dofs + 7] = self.ropehead_pose  # 7
        self.obs_buf[:, 2*self.num_dofs + 7 : 2*self.num_dofs + 10] = self.ropehead_linvel  # 3
        self.obs_buf[:, 2*self.num_dofs + 10 : 2*self.num_dofs + 13] = self.ropehead_angvel  # 3
        self.obs_buf[:, 2*self.num_dofs+13 : 2*self.num_dofs + 20] = self.ropetail_pose  # 7
        self.obs_buf[:, 2*self.num_dofs + 20 : 2*self.num_dofs + 23] = self.ropetail_linvel  # 3
        self.obs_buf[:, 2*self.num_dofs + 23 : 2*self.num_dofs + 26] = self.ropetail_angvel  # 3
        self.obs_buf[:, 2*self.num_dofs + 26] = self.progress_buf.clone()
        if self.act_type in ['dummy_interaction_sphere']:
            self.obs_buf[:, 2*self.num_dofs + 27] = self.is_attached.float()
            self.obs_buf[:, 2*self.num_dofs + 28] = self.another_is_attached.float()
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['rope']:
            ...
            att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'cy1')
            for att in object_attachs:
                att_point = np.array(att['translation'])
                if att_point[1] == 0:
                    att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'cy1')
                else:
                    att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'cy16')
                ...
                att['rotation_matrix'] = att['rotation_matrix'] @ R.from_euler('xyz', [1.57,-1.57,0]).as_matrix()
                att_tm = np.eye(4)
                att_tm[:3, :3] = att_rot
    ...
    def get_states(self):
        ...
        return {
            ...
            'rope_dof': self.dof_states[0, 6:21, 0].clone().cpu().numpy(),
            'rope_dof_vel': self.dof_states[0, 6:21, 1].clone().cpu().numpy(),
            'head_pose': self.ropehead_pose.clone().cpu().numpy(),
            'head_linvel': self.ropehead_linvel.clone().cpu().numpy(),
            'head_angvel': self.ropehead_angvel.clone().cpu().numpy(),
            'tail_pose': self.ropetail_pose.clone().cpu().numpy(),
            'tail_linvel': self.ropetail_linvel.clone().cpu().numpy(),
            'tail_angvel': self.ropetail_angvel.clone().cpu().numpy()
        }
