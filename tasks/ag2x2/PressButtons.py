...
class PressButtons(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.viewer != None:
            cam_pos = gymapi.Vec3(0.7, 0.0, 0.4)
        ...
        if self.repre_type not in ['handcrafted', 'eureka']:
            goal_hand = world2screen(torch.tensor([[0, 0.2, 0.01], [0, -0.2, 0.01]]).unsqueeze(0).to(self.device), 
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name, object_asset_file in object_asset_files.items():
            ...
            object_asset_options.linear_damping = 115
            object_asset_options.angular_damping = 115
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            robot_start_dof = to_torch([0.0, -0.785, 0.0, -2.356, 0.0, 1.571, 0.785, 0.04, 0.04], device=self.device)  # right
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(0.4, 0.4, 0.0)  # right
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 3.14]).as_quat())
            robot_another_start_pose = gymapi.Transform()
            robot_another_start_pose.p = gymapi.Vec3(0.4, -0.4, 0.0)  # left
            robot_another_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 3.14]).as_quat())
        else:
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0, 0.265, 0.065)  # right
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 0.]).as_quat())
            robot_another_start_pose = gymapi.Transform()
            robot_another_start_pose.p = gymapi.Vec3(0, -0.265,  0.065)  # left
        ...
        for object_asset_name in ["green_button", "yellow_button", "blue_button", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.4, 0.0, 0.3)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.05)
        elif self.camera == 'up':
            self.camera_place_pos = gymapi.Vec3(0.3, 0.0, 0.4)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.05)
        elif self.camera == 'down':
            self.camera_place_pos = gymapi.Vec3(0.4, 0.0, 0.2)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.05)
        elif self.camera == 'demo':
            self.camera_props.width = 3840
            self.camera_props.height = 2160
            self.camera_place_pos = gymapi.Vec3(2.0, 0.0, 0.7)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name.endswith('_button'):
                ...
                if object_asset_name == 'green_button':
                    self.green_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'green_joint')
                if object_asset_name == 'yellow_button':
                    self.yellow_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'yellow_joint')
                if object_asset_name == 'blue_button':
                    self.blue_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'blue_joint')
    ...
    def compute_reward(self, actions):
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            camera_images = camera_images.float() / 255.  # [B, 1280, 1280, 3]
            ...
            if self.eureka_seed == 0:
                pressing_green_button_reward = (self.green_dof_pos < -0.007).float()
                pressing_yellow_button_reward = (self.yellow_dof_pos < -0.007).float()
                reward = pressing_green_button_reward + pressing_yellow_button_reward
            elif self.eureka_seed == 1:
                button_dist_tolerance = torch.tensor(0.05)
                button_reward_scale = torch.tensor(100.0)
                green_button_pressed = (torch.sum(torch.abs(self.gripper_pos - torch.tensor([0,0.2,0],device=self.device)),dim=1) <= button_dist_tolerance)
                yellow_button_pressed = (torch.sum(torch.abs(self.gripper_another_pos - torch.tensor([0,-0.2,0],device=self.device)),dim=1) <= button_dist_tolerance)
                green_button_reward = torch.where(green_button_pressed, 100, torch.zeros_like(button_reward_scale,device=self.device))
                yellow_button_reward = torch.where(yellow_button_pressed, 100, torch.zeros_like(button_reward_scale,device=self.device))
                reward = green_button_reward + yellow_button_reward
            elif self.eureka_seed == 2:
                dist_green = torch.norm(self.gripper_pos - torch.tensor([0,0.2,0],device=self.device), dim=-1)
                dist_yellow = torch.norm(self.gripper_another_pos - torch.tensor([0,-0.2,0],device=self.device), dim=-1)
                button_green_pressed = (dist_green < 0.02)
                button_yellow_pressed = (dist_yellow < 0.02)
                reward_dist_green = -dist_green
                reward_dist_yellow = -dist_yellow
                reward_push_buttons = button_green_pressed.float() + button_yellow_pressed.float()
                reward_dist_green = torch.exp(0.1 * reward_dist_green)
                reward_dist_yellow = torch.exp(0.1 * reward_dist_yellow)
                reward_push_buttons = torch.exp(1.0 * reward_push_buttons)
                reward = reward_dist_green + reward_dist_yellow + reward_push_buttons
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError

        goal_button_dof = -0.007
        green_dof_pos = self.green_dof_pos.clone()
        yellow_dof_pos = self.yellow_dof_pos.clone()
        achived_green = torch.norm(green_dof_pos - goal_button_dof) < 0.001
        achived_yellow = torch.norm(yellow_dof_pos - goal_button_dof) < 0.001
        achived_goal = achived_green & achived_yellow
        ...
        self.scores = 0.5 * (green_dof_pos + yellow_dof_pos) / goal_button_dof
        ...
        if self.repre_type == 'handcrafted':
            reward = self.scores
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
            conditions = (self.attach_time > 0) & (self.another_attach_time > 0) & (torch.abs(self.attach_time - self.another_attach_time) <= 15)
            reward *= conditions
        ...
        self.extras['indicator_green'] = green_dof_pos.clone().mean().unsqueeze(0)
        self.extras['indicator_yellow'] = yellow_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.robot_another_dof_pos = self.robot_another_dof_state[..., 0]
        self.robot_another_dof_vel = self.robot_another_dof_state[..., 1]
        self.green_dof_pos = self.dof_states[:, self.green_dof_handle, 0]
        self.green_dof_vel = self.dof_states[:, self.green_dof_handle, 1]
        self.yellow_dof_pos = self.dof_states[:, self.yellow_dof_handle, 0]
        self.yellow_dof_vel = self.dof_states[:, self.yellow_dof_handle, 1]
        self.blue_dof_pos = self.dof_states[:, self.blue_dof_handle, 0]
        self.blue_dof_vel = self.dof_states[:, self.blue_dof_handle, 1]
    ...
    def compute_full_state(self):
        ...
        self.obs_buf[:, self.num_dofs : 2*self.num_dofs] = self.vel_obs_scale * self.dof_states[:, :, 1].clone().detach()
    ...
    def _set_control_stage(self, env_ids, is_attached, attached_body_index, another_is_attached, another_attached_body_index):
        ...
        self.attach_time[env_ids] = -1
        #self.gym.set_rigid_body_color(self.envs[env_ids], 2, 0, gymapi.MESH_VISUAL, gymapi.Vec3(0.1, 0.4, 0))  # purple -> green
        ...
        self.another_attach_time[env_ids] = -1
        #self.gym.set_rigid_body_color(self.envs[env_ids], 3, 0, gymapi.MESH_VISUAL, gymapi.Vec3(1, 0.5, 0))  # purple -> yellow
        for index in env_ids:
            env_index = index.item()
            self.gym.set_rigid_body_color(self.envs[env_index], 2, 0, gymapi.MESH_VISUAL, gymapi.Vec3(0.1, 0.4, 0))  # purple -> green
            self.gym.set_rigid_body_color(self.envs[env_index], 3, 0, gymapi.MESH_VISUAL, gymapi.Vec3(1, 0.5, 0))  # purple -> yellow
    ...
    def pre_physics_step(self, actions):
        ...
        if self.act_type in ['robot_joint_pose']:
            ...
            dof_states[:, self.robot_dof_indices[[2]], 0] = 1.0
            dof_states[:, self.robot_another_dof_indices[[2]], 0] = 1.0
    ...
    def _setup_attachable_body(self, ):
        ...
        self.attach_time = torch.full((self.num_envs,), -1, device=self.device)
        ...
        self.another_attach_time = torch.full((self.num_envs,), -1, device=self.device)
        ...
        self.body_tms = []
        self.related_tms = []
        for object_color in ['green', 'yellow', 'blue']:
            object_name = object_color + '_button'
            object_attachs = pickle.load(open(os.path.join(env_asset_root, 'attachable', f'{object_name}.pkl'), 'rb'))['data']
            ...
            att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], object_color + '_top')  # button top
            for att in object_attachs:
                att_point = att['translation']
                att_point = object_rot @ np.array(att_point) + object_trans
                att['translation'] += [0, 0, 0.12]
                att_rot = att['rotation_matrix']
                att_rot = object_rot @ (att_rot @ R.from_euler('xyz', [0., 0., 1.57]).as_matrix())  # till here, attach pose in world
                att_tm = np.eye(4)
                att_tm[:3, :3] = att_rot
    ...
    def _refresh_attached_states(self):
        ...
        is_local_attached = local_attached < 0.05
        ...
        self.attach_time = torch.where(refresh_attached_body_index, self.progress_buf, self.attach_time)
        ...
        another_is_local_attached = another_local_attached < 0.05
        ...
        self.another_attach_time = torch.where(another_refresh_attached_body_index, self.progress_buf, self.another_attach_time)

        conditions = (self.attach_time > 0) & (self.another_attach_time > 0) & (torch.abs(self.attach_time - self.another_attach_time) <= 15)
        refresh_conditions = refresh_attached_body_index | another_refresh_attached_body_index  # avoid setting colors repeatedly
        conditions = conditions & refresh_conditions
        indices = torch.nonzero(conditions, as_tuple=True)[0]
        if indices.numel() > 0:
            for index in indices:
                env_index = index.item()
                self.gym.set_rigid_body_color(self.envs[env_index], 2, 0, gymapi.MESH_VISUAL, gymapi.Vec3(0.4, 0, 0.7))  # green -> purple
                self.gym.set_rigid_body_color(self.envs[env_index], 3, 0, gymapi.MESH_VISUAL, gymapi.Vec3(0.4, 0, 0.7))  # yellow -> purple
