...
class PushBox(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.viewer != None:
            cam_pos = gymapi.Vec3(1.5, 0.0, 0.7)
        ...
        if self.repre_type not in ['handcrafted', 'eureka']:
            goal_hand = world2screen(torch.tensor([[0.35, 2.0, 0.05], [0.35, -1.8, 0.05]]).unsqueeze(0).to(self.device), 
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose.p = gymapi.Vec3(-0.5, 0.7, 0.0)  # right
            ...
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(-0.155, 0.2, 0.1)  # right
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 0.]).as_quat())
            robot_another_start_pose = gymapi.Transform()
            robot_another_start_pose.p = gymapi.Vec3(-0.155, -0.2, 0.1)  # left
        ...
        for object_asset_name in ["box", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(1.0, 0.0, 0.6)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.1)
        elif self.camera == 'up':
            self.camera_place_pos = gymapi.Vec3(0.8, 0.0, 0.6)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.1)
        elif self.camera == 'down':
            self.camera_place_pos = gymapi.Vec3(1.2, 0.0, 0.6)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.1)
        ...
        for i in range(self.num_envs):
            ...
            asset_options.disable_gravity = True
            asset_options.thickness = 0.001
            cube_geom = self.gym.create_box(self.sim, 0.25, 0.55, 0.01, asset_options)
            pose = gymapi.Transform()
            pose.p = gymapi.Vec3(0.4, 0.1, 0.00163)
            pose.r = gymapi.Quat(0, 0, 0, 1)
            cube_color = gymapi.Vec3(1, 0, 0)  # red (1, 0, 0)
            actor_handle = self.gym.create_actor(env_ptr, cube_geom, pose, "mark", 0)
            self.gym.set_rigid_body_color(env_ptr, actor_handle, 0, gymapi.MESH_VISUAL, cube_color)
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'box':
                    self.box_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 0)
        ...
        self.num_rigid_bodies = self.num_robot_bodies * 2 + self.num_object_bodies + 1  # for mark
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            if self.eureka_seed == 0:
                dist_temp = 0.1
                dist_to_goal = torch.norm(self.box_pose[:, :2] - torch.tensor([0.4,0.1],device=self.device), dim=-1)
                reward = - torch.exp(dist_to_goal / dist_temp)
            elif self.eureka_seed == 1:
                temp_threshold = torch.tensor(0.01, device=self.box_pose.device)
                diff = self.box_pose -  torch.tensor([0.4,0.1,0.1,0,0,0,1], device=self.box_pose.device)
                distance = torch.norm(diff, dim=-1)
                reward = - torch.exp(distance / temp_threshold)
            elif self.eureka_seed == 2:
                box_pos = self.box_pose[:, 0:3]
                distance_to_target = torch.norm(box_pos - torch.tensor([0.4,0.1,0.1],device=self.device), dim=-1)
                temperature = torch.tensor(1.0).to(self.box_pose.device)
                reward = torch.exp(-temperature * distance_to_target)
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError

        goal_box_pose = torch.tensor([0.4, 0.1, 0.1, 0., 0., 0., 1.], device=self.device)
        box_pose = self.box_pose.clone()
        achived_goal = torch.norm(box_pose - goal_box_pose, dim=1) < 0.05
        ...
        self.scores = 1 - torch.norm(box_pose - goal_box_pose, dim=1) / 0.41
        ...
        self.extras['indicator_x'] = box_pose.clone()[:, 0].mean().unsqueeze(0)
        self.extras['indicator_y'] = box_pose.clone()[:, 1].mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.box_pose = self.rigid_body_states[:, self.box_body_handle, 0:7]
        self.box_linvel = self.rigid_body_states[:, self.box_body_handle, 7:10]
        self.box_angvel = self.rigid_body_states[:, self.box_body_handle, 10:13]
    ...
    def compute_full_state(self):
        ...
        self.obs_buf[:, 2*self.num_dofs : 2*self.num_dofs + 7] = self.box_pose  # 7
        self.obs_buf[:, 2*self.num_dofs + 7 : 2*self.num_dofs + 10] = self.box_linvel  # 3
        self.obs_buf[:, 2*self.num_dofs + 10 : 2*self.num_dofs + 13] = self.box_angvel  # 3
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['box']:
            ...
            att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'box_body')  # box body
            for att in object_attachs:
                att['translation'][0], att['translation'][1] = att['translation'][1], att['translation'][0]
                att_point = np.array(att['translation'])
                pow_new = np.array(att['rotation_matrix'])
                ...
                att['rotation_matrix'] = R.from_quat(np.array([0.707, 0, 0, 0])).as_matrix()
                att_tm = np.eye(4)
                att_tm[:3, :3] = att_rot
    ...
    def step_plan(self, actions, actions1):
        ...
        # action for gripper
        for i_env in range(self.num_envs):
            dof_effort = np.ones(self.num_dofs, dtype=np.float32) * gripper_effort
