...
class CloseScissors(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.repre_type not in ['handcrafted', 'eureka']:
            goal_hand = world2screen(torch.tensor([[0.17, 0.02, 0.05], [0.15, -0.12, 0.05]]).unsqueeze(0).to(self.device), 
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0.15, 0.2, 0.15)  # right
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 0.]).as_quat())
            robot_another_start_pose = gymapi.Transform()
            robot_another_start_pose.p = gymapi.Vec3(0.15, -0.2, 0.18)  # left
        ...
        for object_asset_name in ["scissors", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.5, 0.0, 0.4)
            self.camera_lookat_pos = gymapi.Vec3(0, 0, 0)
        elif self.camera == 'up':
            self.camera_place_pos = gymapi.Vec3(0.5, 0.0, 0.45)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'scissors':
                    self.scissors_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'joint_0')
                if object_asset_name == 'scissors':
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            scissors_dof_pos = self.scissors_dof_pos.clone()
            reward = scissors_dof_pos / -0.55
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                target_pos_reached = torch.abs(self.scissors_dof_pos + 0.55)
                distance_temperature = torch.tensor(1.0)
                reward = torch.exp(-distance_temperature * target_pos_reached)
            elif self.eureka_seed == 1:
                pos_diff = torch.norm(self.scissors_dof_pos + 0.55, dim=-1)
                reward = - pos_diff
                temperature=0.03
                reward = torch.exp(reward / temperature)
            elif self.eureka_seed == 2:
                close_reward: torch.Tensor = -self.scissors_dof_pos
                close_reward_normalized = torch.exp(close_reward / 0.1)
                reward = close_reward_normalized 
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        
        goal_scissors_dof_pos = -0.55
        scissors_dof_pos = self.scissors_dof_pos.clone()
        achived_goal = torch.abs(goal_scissors_dof_pos - scissors_dof_pos) < 0.05
        ...
        self.scores = scissors_dof_pos / goal_scissors_dof_pos
        ...
        self.extras['indicator'] = self.scissors_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.robot_another_dof_pos = self.robot_another_dof_state[..., 0]
        self.robot_another_dof_vel = self.robot_another_dof_state[..., 1]
        self.scissors_dof_pos = self.dof_states[:, self.scissors_dof_handle, 0]
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['scissors']:
            ...
            camt = compute_camera_transform(torch.tensor([0.05, 0.0, -1.0]), torch.tensor([0.0, 0.0, 0.0]))
            for att in object_attachs:
                ...
                att['translation'] = np.array(att_point)*2.0
                ...
                if att_point[0] < 0:  # remove grasp points on the blade
                    continue
                ...
                if att_point[1] > 0.:  # up, right
                    att['translation'] += [-0.02, 0.03, 0.09]
                    att['rotation_matrix'] = att['rotation_matrix'] @ R.from_euler('xyz', [0, -1.57, 1.57]).as_matrix()
                    att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'link_0')
                elif att_point[1] < 0:  # down, left
                    att['translation'] += [0.05, 0, 0.03]
                    att['rotation_matrix'] = R.from_quat(np.array([0.707, 0.707, 0, 0])).as_matrix()
