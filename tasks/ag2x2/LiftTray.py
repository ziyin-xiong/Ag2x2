...
class LiftTray(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.repre_type not in ['handcrafted', 'eureka']:
            goal_hand = world2screen(torch.tensor([[0, 0.2, 0.31], [0, -0.2, 0.31]]).unsqueeze(0).to(self.device), 
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name, object_asset_file in object_asset_files.items():
            ...
            if object_asset_name in ["tray", "cube"]:
                object_asset_options.fix_base_link = False
            else:
                object_asset_options.fix_base_link = True
            object_asset_options.disable_gravity = False
            if object_asset_name == "tray":
        ...
        if self.is_planning or self.act_type in ['robot_joint_pose']:
            ...
            robot_start_pose = gymapi.Transform()
            robot_start_pose.p = gymapi.Vec3(0., 0.3,  0.2)  # right
            robot_start_pose.r = gymapi.Quat(*R.from_euler('xyz', [0., 0., 0.]).as_quat())
            robot_another_start_pose = gymapi.Transform()
            robot_another_start_pose.p = gymapi.Vec3(0,  -0.3,  0.2)  # left
        ...
        for object_asset_name in ["tray", "cube", "drawer_legs", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.3, 0.0, 0.35)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.15)
        elif self.camera == 'up':
            self.camera_place_pos = gymapi.Vec3(0.3, 0.0, 0.45)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.15)
        elif self.camera == 'down':
            self.camera_place_pos = gymapi.Vec3(0.35, 0.0, 0.35)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.15)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                for object_shape_prop in object_shape_props:
                    object_shape_prop.friction = 10.0
                ...
                if object_asset_name == 'tray':
                    self.tray_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 0)
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            if self.eureka_seed == 0:
                tray_height = self.tray_pose[..., 2]
                height_difference = 0.31 - tray_height
                height_temperature = 0.01
                height_reward = torch.exp(-height_difference / height_temperature)
                raw_reward = height_reward
                reward = torch.tanh(raw_reward)
            elif self.eureka_seed == 1:
                pos_diff = torch.norm(self.tray_pose[:, 0:3] - torch.tensor([0,0,0.31],device=self.device), dim=-1)
                ori_diff = 1.0 - torch.nn.functional.cosine_similarity(self.tray_pose[:, 3:], torch.tensor([0,0,0,1],device=self.device), dim=-1)
                linvel_diff = torch.norm(self.tray_linvel, dim=-1)
                angvel_diff = torch.norm(self.tray_angvel, dim=-1)
                pos_reward = torch.exp(-1.0 * pos_diff)
                ori_reward = torch.exp(-1.0 * ori_diff)
                linvel_reward = torch.exp(-1.0 * linvel_diff)
                angvel_reward = torch.exp(-1.0 * angvel_diff)
                reward = pos_reward + ori_reward + linvel_reward + angvel_reward
            elif self.eureka_seed == 2:
                tray_vel_temp: float = 0.1
                tray_pos_temp: float = 1.0
                diff_height = 0.31 - self.tray_pose[:, 2] 
                r_tray_vel = torch.exp(-tray_vel_temp * (torch.norm(self.tray_linvel, dim=-1)))
                r_height_diff = torch.exp(-tray_pos_temp * (torch.abs(diff_height)))
                reward = r_tray_vel + r_height_diff
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError

        goal_tray_pose = torch.tensor([0, 0, 0.3104, 0, 0, 0, 1], device=self.device)  # 水平地悬在空中
        tray_pose = self.tray_pose.clone()
        achived_goal = torch.norm(tray_pose - goal_tray_pose, dim=1) < 0.1
        ...
        self.scores = 1 - torch.norm(tray_pose[:, :3] - goal_tray_pose[:3], dim=1) / 0.2
        ...
        self.extras['indicator'] = tray_pose.clone()[:, 2].mean().unsqueeze(0)  # only height
    ...
    def compute_observations(self):
        ...
        self.tray_pose = self.rigid_body_states[:, self.tray_body_handle, 0:7]
        self.tray_linvel = self.rigid_body_states[:, self.tray_body_handle, 7:10]
        self.tray_angvel = self.rigid_body_states[:, self.tray_body_handle, 10:13]
    ...
    def compute_full_state(self):
        ...
        self.obs_buf[:, 2*self.num_dofs : 2*self.num_dofs + 7] = self.tray_pose  # 7
        self.obs_buf[:, 2*self.num_dofs + 7 : 2*self.num_dofs + 10] = self.tray_linvel  # 3
        self.obs_buf[:, 2*self.num_dofs + 10 : 2*self.num_dofs + 13] = self.tray_angvel  # 3
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['tray']:
            ...
            att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'tray_body')  # tray body
            for att in object_attachs:
                ...
                if att_point[1] > 0:  # right
                    ...
                    att['rotation_matrix'] = R.from_quat(np.array([0.707, 0, 0, -0.707])).as_matrix()
                att_tm = np.eye(4)
                att_tm[:3, :3] = att_rot
