...
class CloseDoorInward(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.viewer != None:
            cam_pos = gymapi.Vec3(2.0, 0.0, 0.5)
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name in ["door_open", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.9, 0, 0.4)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'door_open':
                    self.door_right_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'joint_1')
                    self.door_left_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'joint_2')
                if object_asset_name == 'door_open':
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            reward = (3.14-door_left_dof_pos-door_right_dof_pos)/3.14
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                temp = torch.tensor(0.01)
                door_pos_total = torch.norm(self.door_right_dof_pos + self.door_left_dof_pos)
                door_angle_reward = torch.exp(-temp * door_pos_total)
                reward = door_angle_reward
            elif self.eureka_seed == 1:
                temp_d: torch.Tensor = torch.ones(1) * 0.5
                left_distance = torch.abs(self.door_left_dof_pos)
                right_distance = torch.abs(self.door_right_dof_pos)
                left_reward = torch.exp(-temp_d.to(self.device) * left_distance)
                right_reward = torch.exp(-temp_d.to(self.device) * right_distance)
                reward = torch.mean(torch.stack([left_reward, right_reward]), dim=0)
            elif self.eureka_seed == 2:
                reward_components = {}
                temp1 = torch.tensor(0.1, device=self.door_right_dof_pos.device)
                temp2 = torch.tensor(0.1, device=self.door_left_dof_pos.device)
                reward_components['right_door_close_reward'] = (torch.exp(-temp1 * torch.abs(self.door_right_dof_pos - 0))).mean()
                reward_components['left_door_close_reward'] = (torch.exp(-temp2 * torch.abs(self.door_left_dof_pos - 0))).mean()
    ...
    def _setup_attachable_body(self, ):
        ...
        self.body_tms = []
        self.related_tms = []
        object_name = 'door_open'
        object_attachs = pickle.load(open(os.path.join(env_asset_root, 'attachable', 'doorr.pkl'), 'rb'))['data']
        ...
        camt = compute_camera_transform(torch.tensor([-0.25, -0.25, 0]), torch.tensor([-0.25, 0, 0]))
        for att in object_attachs:
            ...
            att_point[2]*=-1
            att_point[0] -= 0.05  # till here, relative pose to object
            att['translation'] = np.array(att_point)
            att['rotation_matrix'] = R.from_quat(np.array([0.707,0,0,-0.707])).as_matrix() @ R.from_euler('xyz', [0,0,1.57]).as_matrix()  # update from pickle file to real relative pose
            ...
            att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'link_1')  # right
        ...
        object_attachs = pickle.load(open(os.path.join(env_asset_root, 'attachable', 'doorl.pkl'), 'rb'))['data']
        object_trans = np.array(env_asset_place[object_name]['pos'])
        object_rot = np.array(env_asset_place[object_name]['rot'])
        object_rot = R.from_euler('xyz', object_rot).as_matrix()
        camt = compute_camera_transform(torch.tensor([-0.25, 0.25, 0]), torch.tensor([-0.25, 0, 0]))
        for att in object_attachs:
            ...
            att_rot = R.from_matrix(att_rot).as_quat()
            self._attachable_points.append(att_point)
            att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'link_2')  # left
            self._attachable_handles.append(att_rb_handle)
            body_tm = np.eye(4)  # initial attached rigid part pose matrix in world
            body_tm[:3, 3] = self.rigid_body_states[0, att_rb_handle, :3].cpu().numpy()
            body_tm[:3, :3] = R.from_quat(self.rigid_body_states[0, att_rb_handle, 3:7].cpu().numpy()).as_matrix()
