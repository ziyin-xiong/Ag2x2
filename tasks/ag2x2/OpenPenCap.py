...
class OpenPenCap(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.act_type in ['robot_joint_pose']:
            self.num_obs_dict = {
                ...
                "full_state": 19 * 2 + 13 + 1,  # 12*2 for both hands, 1 for progress, +1+1 for each hand's attached
            }
        else:
            self.num_obs_dict = {
                "robot_state": 8 * 4 + 1,
                # "full_state": 9 + 9 + 1 + 1,  #* (9 + 9) for robot joint pos and vel, (1 + 1) for manipulated object pos and vel
                "full_state": 8 * 2 + 13 + 1 + 1 + 1,  # num_dofs = 8
            }
        ...
        if self.viewer != None:
            cam_pos = gymapi.Vec3(0.65, 0.0, 0.3)
        ...
        if self.repre_type not in ['handcrafted', 'eureka']:
            goal_hand = world2screen(torch.tensor([[0.0, 0.48, 0.07], [0.0, -0.12, 0.07]]).unsqueeze(0).to(self.device), 
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name, object_asset_file in object_asset_files.items():
            ...
            if object_asset_name == "pen":
                object_asset_options.fix_base_link = False
            else:
                object_asset_options.fix_base_link = True
            object_asset_options.disable_gravity = False
            object_asset_options.linear_damping = 30
            object_asset_options.angular_damping = 30
        ...
        for object_asset_name in ["pen", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.7, 0.0, 0.3)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.1)
        elif self.camera == 'up':
            self.camera_place_pos = gymapi.Vec3(0.6, 0.0, 0.4)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.1)
        elif self.camera == 'down':
            self.camera_place_pos = gymapi.Vec3(0.7, 0.0, 0.2)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.1)
            ...
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.0)
        else:
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'pen':
                    self.pen_dof_handle = self.gym.find_actor_dof_handle(env_ptr, object_handle, 'joint_0')
                    self.pen_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 1)
                if object_asset_name == 'pen':
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            pen_dof_pos = self.pen_dof_pos.clone()
            reward = torch.min(pen_dof_pos / 0.3, torch.ones(self.num_envs, device=self.device)) - torch.sum(torch.abs(self.pen_pose-torch.tensor([0,0,0.028,0.7,0,0,0.7],device=self.pen_pose.device)), dim=1)
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                pos_temp = torch.tensor(0.5)
                linvel_temp = torch.tensor(0.2)
                angvel_temp = torch.tensor(0.2)
                dof_pos_temp = torch.tensor(0.1)
                target_pos_temp = torch.tensor(4.0)
                pose_diff = torch.sum((self.pen_pose - torch.tensor([0., 0., 0.028, 0.7, 0., 0., 0.7],device=self.pen_pose.device))**2, dim=-1)
                linear_vel_reward = -torch.sum(self.pen_linvel**2, dim=-1)
                angular_vel_reward = -torch.sum(self.pen_angvel**2, dim=-1)
                dof_pos_reward = -torch.abs(self.pen_dof_pos)
                target_diff = torch.sum((self.gripper_pos - self.gripper_another_pos)**2, dim=-1)
                reward = torch.exp(-pose_diff / pos_temp) + torch.exp(linear_vel_reward / linvel_temp) + torch.exp(angular_vel_reward / angvel_temp) + torch.exp(dof_pos_reward / dof_pos_temp) + torch.exp(-target_diff / target_pos_temp)
            elif self.eureka_seed == 1:
                vel_temp: torch.Tensor = torch.ones(self.num_envs) * 0.1
                angvel_temp: torch.Tensor = torch.ones(self.num_envs) * 0.1
                pen_temp: torch.Tensor = torch.ones_like(self.pen_dof_pos) * 0.1
                pose_temp: torch.Tensor = torch.ones(self.num_envs) * 0.1
                pen_reward: torch.Tensor = torch.exp(-pen_temp.to(self.device) * torch.abs(self.pen_dof_pos)) 
                pose_reward: torch.Tensor = torch.exp(-pose_temp.to(self.device) * torch.norm(torch.tensor(self.gripper_pos,device=self.device) - torch.tensor(self.gripper_another_pos,device=self.device), dim=1))
                vel_penalty: torch.Tensor = torch.exp(-vel_temp.to(self.device) * torch.norm(self.pen_linvel, dim=1))
                angvel_penalty: torch.Tensor = torch.exp(-angvel_temp.to(self.device) * torch.norm(self.pen_angvel, dim=-1))
                reward: torch.Tensor = pen_reward + pose_reward - vel_penalty - angvel_penalty
            elif self.eureka_seed == 2:
                pen_open_reward = 1.0
                pen_failure_penalty = -1.0
                open_pen_reward = torch.where(self.pen_dof_pos > 0.3, pen_open_reward, 0.0)
                open_pen_penalty = torch.where(self.pen_dof_pos <= 0.25, pen_failure_penalty, 0.0)
                reward = open_pen_reward + open_pen_penalty
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError
        
        goal_pen_dof_pos = 0.3
        pen_dof_pos = self.pen_dof_pos.clone()
        achived_goal = torch.abs(goal_pen_dof_pos - pen_dof_pos) < 0.02
        ...
        self.scores = pen_dof_pos / goal_pen_dof_pos
        ...
        self.extras['indicator'] = self.pen_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.robot_another_dof_pos = self.robot_another_dof_state[..., 0]
        self.robot_another_dof_vel = self.robot_another_dof_state[..., 1]
        self.pen_dof_pos = self.dof_states[:, self.pen_dof_handle, 0]
        ...
        self.pen_pose = self.rigid_body_states[:, self.pen_body_handle, 0:7]
        self.pen_linvel = self.rigid_body_states[:, self.pen_body_handle, 7:10]
        self.pen_angvel = self.rigid_body_states[:, self.pen_body_handle, 10:13]
    ...
    def compute_full_state(self):
        ...
        self.obs_buf[:, 2*self.num_dofs : 2*self.num_dofs + 7] = self.pen_pose  # 7
        self.obs_buf[:, 2*self.num_dofs + 7 : 2*self.num_dofs + 10] = self.pen_linvel  # 3
        self.obs_buf[:, 2*self.num_dofs + 10 : 2*self.num_dofs + 13] = self.pen_angvel  # 3
        self.obs_buf[:, 2*self.num_dofs + 13] = self.progress_buf.clone()
        if self.act_type in ['dummy_interaction_sphere']:
            self.obs_buf[:, 2*self.num_dofs + 14] = self.is_attached.float()
            self.obs_buf[:, 2*self.num_dofs + 15] = self.another_is_attached.float()
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['pen']:
            ...
            object_rot = np.array([1.57, -1.57, 0.])  # rotate the attach points to the upper side
            object_rot = R.from_euler('xyz', object_rot).as_matrix()
            camt = compute_camera_transform(torch.tensor([1.0, 0.0, 0.0]), torch.tensor([0.0, 0.0, 0.0]))
            for att in object_attachs:
                ...
                att_point[1] *= -1.0
                att_point[2] *= -1.0
                att['translation'] = np.array(att_point)*2  # update from pickle file to real relative pose

                att_point = object_rot @ np.array(att_point) + object_trans
                att_rot = object_rot @ (np.array(pos_new[:3, :3]) @ R.from_euler('xyz', [0., 0., 1.57]).as_matrix())  # till here, attach pose in world
                att_tm = np.eye(4)
                att_tm[:3, :3] = att_rot
                att_point *= 2.0
                att_tm[:3, 3] = att_point
                att_rot = R.from_matrix(att_rot).as_quat()
                if att_point[1] > 0.1:  # > 0.05 for cap
                    att['translation'] += [0.08, 0, 0.05]
                    att['rotation_matrix'] = np.array(pos_new[:3, :3]) @ R.from_euler('xyz', [-1.57, -0.7, 0]).as_matrix()
                    att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'link_0')
                elif att_point[1] < 0.:  # < 0 for body
                    att['translation'] += [0.08, 0.02, 0]
                    att['rotation_matrix'] = np.array(pos_new[:3, :3]) @ R.from_euler('xyz', [-1.57, 1.57, 0]).as_matrix()
                    att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'link_1')
                else:
                    continue
