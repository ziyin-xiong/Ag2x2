...
class LiftPot(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.repre_type not in ['handcrafted', 'eureka']:
            goal_hand = world2screen(torch.tensor([[0, 0.175, 0.32], [0, -0.175, 0.32]]).unsqueeze(0).to(self.device), 
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name in ["pot", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.5, 0.0, 0.3)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0)
        elif self.camera == 'up':
            self.camera_place_pos = gymapi.Vec3(0.6, 0.0, 0.3)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.0)
        elif self.camera == 'down':
            self.camera_place_pos = gymapi.Vec3(0.47, 0.0, 0.25)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.0)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                for object_shape_prop in object_shape_props:
                    object_shape_prop.friction = 3.0
                ...
                if object_asset_name == 'pot':
                    self.pot_body_handle = self.gym.get_actor_rigid_body_handle(env_ptr, object_handle, 1)

                if object_asset_name == 'pot':
                    dof_states = self.gym.get_actor_dof_states(env_ptr, object_handle, gymapi.STATE_ALL)
                    dof_states['pos'][0] = 0.0  # dof is for open pot lid
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            goal_pot_pose = torch.tensor([0, 0, 0.25, 0.5, -0.5, -0.5, 0.5], device=self.device)
            pot_pose = self.pot_pose.clone()
            reward = 1 - torch.norm(goal_pot_pose - pot_pose, dim=1) / 0.185
            reward = torch.where(reward < 0, torch.exp(reward) - 1, 10 * (torch.exp(2 * reward) - 1))
        elif self.repre_type in ['eureka']:
            if self.eureka_seed == 0:
                distance_reward_scale: float = 1.0
                orientation_reward_scale: float = 0.5
                height_reward_scale: float = 0.5
                temperature_distance: float = 0.1
                temperature_orientation: float = 0.1
                temperature_height: float = 0.1
                distance = torch.norm(self.pot_pose[:, :3] - torch.tensor([0, 0, 0.25], device=self.device), dim=-1)
                orientation_difference = torch.abs(1 - torch.sum(self.pot_pose[:, 3:] * torch.tensor([0.5, -0.5, -0.5, 0.5], device=self.device), dim=-1))
                normalized_height = (self.pot_pose[:, 2] - 0.068) / (0.3 - 0.068)
                height_reward = torch.clip(normalized_height, 0.0, 1.0)
                reward_distance = torch.exp(-temperature_distance * distance) * distance_reward_scale
                reward_orientation = torch.exp(-temperature_orientation * orientation_difference) * orientation_reward_scale
                reward_height = torch.exp(temperature_height * (height_reward - 1)) * height_reward_scale
                reward = reward_distance + reward_orientation + reward_height
            elif self.eureka_seed == 1:
                height_reward_scale: float = 10.0
                orientation_reward_scale: float = 5.0
                gripper_close_reward_scale: float = 5.0
                target_height_tensor: torch.Tensor = torch.tensor(0.25, dtype=torch.float32, device=self.pot_pose.device)
                height_dist = torch.relu(self.pot_pose[:, 2] - target_height_tensor)
                height_reward = torch.exp(-height_reward_scale * height_dist)
                desired_orientation: torch.Tensor = torch.tensor([0.5, -0.5, -0.5, 0.5], dtype=torch.float32, device=self.pot_pose.device)
                orientation_diff = torch.norm(self.pot_pose[:, 3:] - desired_orientation.unsqueeze(0), dim=1)
                orientation_reward = torch.exp(-orientation_reward_scale * orientation_diff)
                reward = height_reward + orientation_reward
            elif self.eureka_seed == 2:
                distance_temperature: float = 0.1
                dist_error = torch.norm(self.pot_pose[:, :3] - torch.tensor([0, 0, 0.25], dtype=torch.float32, device=self.pot_pose.device), dim=-1)
                reward_dist = -torch.exp(dist_error / distance_temperature)  
                reward = reward_dist
            else:
                raise NotImplementedError
        else:
        ...
        goal_pot_pose = torch.tensor([0, 0, 0.25, 0.5, -0.5, -0.5, 0.5], device=self.device)  # 水平地悬在空中
        pot_pose = self.pot_pose.clone()
        achived_goal = torch.norm(pot_pose - goal_pot_pose, dim=1) < 0.1
        ...
        self.scores = 1 - torch.norm(pot_pose[:, :3] - goal_pot_pose[:3], dim=1) / 0.185
        ...
        self.extras['indicator'] = pot_pose.clone()[:, 2].mean().unsqueeze(0)  # only height
    ...
    def compute_observations(self):
        ...
        self.pot_pose = self.rigid_body_states[:, self.pot_body_handle, 0:7]
        self.pot_linvel = self.rigid_body_states[:, self.pot_body_handle, 7:10]
        self.pot_angvel = self.rigid_body_states[:, self.pot_body_handle, 10:13]
    ...
    def compute_full_state(self):
        ...
        self.obs_buf[:, 2*self.num_dofs : 2*self.num_dofs + 7] = self.pot_pose  # 7
        self.obs_buf[:, 2*self.num_dofs + 7 : 2*self.num_dofs + 10] = self.pot_linvel  # 3
        self.obs_buf[:, 2*self.num_dofs + 10 : 2*self.num_dofs + 13] = self.pot_angvel  # 3
    ...
    def pre_physics_step(self, actions):
        ...
        if self.act_type in ['robot_joint_pose']:
            ...
            dof_states[:, self.robot_dof_indices[[2]], 0] = 2
            dof_states[:, self.robot_another_dof_indices[[2]], 0] = 2
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['pot']:
            ...
            att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'link_1')  # pot body
            for att in object_attachs:
                att_point = np.array(att['translation'])
                att_point = object_rot @ att_point + object_trans
                att_rot = object_rot @ (att['rotation_matrix'] @ R.from_euler('xyz', [0., 0., 1.57]).as_matrix())  # till here, attach pose in world
                if att_point[1] > 0:  # right:
                    att['rotation_matrix'] = att['rotation_matrix'] @ R.from_euler('xyz', [-1.57, 0, 0]).as_matrix()
                else:  # left
                    att['rotation_matrix'] = att['rotation_matrix'] @ R.from_euler('xyz', [1.57, 0, 0]).as_matrix()
                att_tm = np.eye(4)
                att_tm[:3, :3] = att_rot
