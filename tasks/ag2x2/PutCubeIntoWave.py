...
class PutCubeIntoWave(BaseTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, camera, headless):
        ...
        if self.act_type in ['robot_joint_pose']:
            self.num_obs_dict = {
                "full_state": 19*2 + 13 + 1,
            }
        else:
            self.num_obs_dict = {
                "full_state": 7*2 + 7 + 3 + 3 + 1 + 1 + 1,
            }
        ...
        if self.repre_type not in ['handcrafted', 'eureka']: 
            goal_hand = world2screen(torch.tensor([[0, -0.1, 0.12], [0.56,-0.65,0.25]]).unsqueeze(0).to(self.device), 
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        for object_asset_name, object_asset_file in object_asset_files.items():
            ...
            object_asset_options.linear_damping = 5
            object_asset_options.angular_damping = 5
            if object_asset_name == 'cube':
                object_asset_options.fix_base_link = False
                object_asset_options.linear_damping = 20
                object_asset_options.angular_damping = 10
        ...
        for object_asset_name in ["microwave", "cube", "room1", "room2"]:
        ...
        if self.camera in [None]:
            ...
            self.camera_place_pos = gymapi.Vec3(0.6, 0.0, 0.2)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.1)
        elif self.camera == 'down':
            self.camera_place_pos = gymapi.Vec3(0.6, 0, 0.1)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.1)
        elif self.camera == 'up':
            self.camera_place_pos = gymapi.Vec3(0.6, 0, 0.3)
            self.camera_lookat_pos = gymapi.Vec3(0.0, 0.0, 0.1)
        ...
        for i in range(self.num_envs):
            ...
            for object_asset_name in object_assets.keys():
                ...
                if object_asset_name == 'microwave':
                    self.wave_dof_handle = self.gym.get_actor_dof_handle(env_ptr, object_handle, 0)
                if object_asset_name == 'cube':
    ...
    def compute_reward(self, actions):
        ...
        if self.repre_type in ["r3m", "ag2manip", "vip", ]:
            ...
            if self.eureka_seed == 0:
                dist_cube_wave = torch.norm(self.cube_pose[..., 0:3] - torch.tensor([0,0,0],device=self.device), dim=-1)
                g1_dist = torch.norm(self.gripper_pos[..., 0:3] - self.cube_pose[..., 0:3], dim=-1)
                reward_dist = -dist_cube_wave
                reward_g1 = -g1_dist
                temp_parameter_dist = 100.0
                temp_parameter_g1 = 100.0
                reward_dist = torch.exp(reward_dist / temp_parameter_dist)
                reward_g1 = torch.exp(reward_g1 / temp_parameter_g1)
                reward = reward_dist
            elif self.eureka_seed == 1:
                distance_cube_wave = torch.dist(self.cube_pose[:,:3], torch.tensor([0,0,0],device=self.device), 2)
                distance_reward = torch.exp(-distance_cube_wave)
                linvel_cube = torch.norm(self.cube_linvel, dim=-1)
                velocity_reward = torch.exp(-linvel_cube)
                angvel_cube = torch.norm(self.cube_angvel, dim=-1)
                rotation_reward = torch.exp(-angvel_cube)
                reward = 0.5 * distance_reward + 0.25 * velocity_reward + 0.25 * rotation_reward
            elif self.eureka_seed == 2:
                pos_weight = 1.0
                vel_weight = 0.1
                ang_weight = 0.1
                pos_distance = torch.norm(self.cube_pose[..., 0:3] - torch.tensor([0,-0.1,0.12],device=self.device), dim=-1)
                linvel_cost = torch.norm(self.cube_linvel, dim=-1)
                angvel_cost = torch.norm(self.cube_angvel, dim=-1)
                reward = -pos_weight * pos_distance - vel_weight * linvel_cost - ang_weight * angvel_cost
            else:
                raise NotImplementedError
        else:
            raise NotImplementedError

        goal_wave_dof_pos = -2.0
        wave_dof_pos = self.wave_dof_pos.clone()
        achived_pull_goal = torch.abs(goal_wave_dof_pos - wave_dof_pos) < 0.05
        goal_cube_pos = torch.tensor([0.0, -0.1, 0.12], device=self.device)  # 0.4 0.2 0.025
        cube_pos = self.cube_pose.clone()[:, :3]
        achived_put_goal = torch.norm(cube_pos - goal_cube_pos, dim=1) < 0.1
        ...
        self.scores = 0.5 * wave_dof_pos / goal_wave_dof_pos + 0.5 * (1 - torch.norm(cube_pos - goal_cube_pos, dim=1) / 0.55)
        ...
        self.extras['indicator_pull'] = wave_dof_pos.clone().mean().unsqueeze(0)
    ...
    def compute_observations(self):
        ...
        self.robot_another_dof_pos = self.robot_another_dof_state[..., 0]
        self.robot_another_dof_vel = self.robot_another_dof_state[..., 1]
        self.wave_dof_pos = self.dof_states[:, self.wave_dof_handle, 0]
    ...
    def compute_robot_state(self):
        ...
        18 - 26     another robot joint dofs position
        27 - 35     another robot joint dofs velocity
        """
    ...
    def _setup_attachable_body(self, ):
        ...
        for object_name in ['microwave']:
            ...
            camt = compute_camera_transform(torch.tensor([0.0, 1.0, 0.0]), torch.tensor([0.0, 0.0, 0.0]))
            for att in object_attachs:
                ...
                att_point[1] *= -1.0
                att_point[2] *= -1.0  # till here, attach pose relative to object
                ...
                att_rb_handle = self.gym.find_actor_rigid_body_handle(self.envs[0], self.objects_handle[object_name], 'microdoorroot')
        ...
        for object_name in ['cube2']:
            ...
            for att in object_attachs:
                ...
                att_point = object_rot @ np.array(att_point) + object_trans
                att_rot = object_rot @ (att['rotation_matrix'])  # till here, attach pose in world
                att['rotation_matrix'] = att['rotation_matrix'] @ R.from_euler('xyz', [1.57, 1.57, -1.57]).as_matrix()
                att_tm = np.eye(4)
                att_tm[:3, :3] = att_rot
                att_tm[:3, 3] = att_point
