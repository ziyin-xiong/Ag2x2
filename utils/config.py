...
def load_cfg(args, use_rlg_config=False):
    ...
    with open(os.path.join(os.getcwd(), args.cfg_repre), 'r') as f:
        cfg_repre = yaml.load(f, Loader=yaml.SafeLoader)
    
    if cfg_repre is not None:
        cfg['repre'] = cfg_repre

    # Override the general reward specification
    cfg["env"]["rewardType"] = "efficiency"
    if args.plain_reward:
        cfg["env"]["rewardType"] = "plain"

    # Override the action type if args.panda == True
    if args.panda:
        cfg["env"]["actionType"] = "robot_joint_pose"
    ...
    cfg["env"]["enableDebugVis"] = args.debug_vis
    cfg["name"] = args.task
    ...
    return cfg, cfg_train, logdir, cfg_repre

def load_plan_cfg(args):
    with open(os.path.join(os.getcwd(), args.cfg_env), 'r') as f:
        cfg = yaml.load(f, Loader=yaml.SafeLoader)
    with open(os.path.join(os.getcwd(), args.cfg_repre), 'r') as f:
        cfg_repre = yaml.load(f, Loader=yaml.SafeLoader)
    ...
    cfg["env"]["numEnvs"] = 1
    cfg["env"]["actionType"] = "end_effector_pose"
    if args.agentago:
        cfg["env"]["asset"]["agent"]["franka"] = cfg["env"]["asset"]["agent"]["franka-wovis"]

    # Hardcode to set the episode length to inf
    cfg["env"]["episodeLength"] = float('inf')

    cfg['traj_path'] = args.traj_path
    cfg["env"]["enableDebugVis"] = args.debug_vis
    cfg["name"] = args.task
    cfg["headless"] = args.headless
    cfg['planning'] = True
    ...
    args.logdir = os.path.join(os.path.dirname(args.traj_path), "planning")
    logdir = args.logdir
    log_id = args.logdir
    if args.experiment != 'Base':
        if args.metadata:
...
def get_args(benchmark=False, use_rlg_config=False, task_name="", algo=""):
    custom_parameters = [
        ##* START for PLANNING
        {"name": "--plan", "action": "store_true", "default": False,
            "help": "Run transfered trajectory, no training & planning"},
        {"name": "--cfg_plan", "type": str,
            "default": "Base"},
        {"name": "--traj_path", "type": str, "required": True,
            "help": "Dummy trajectory path for planning"},
        {"name": "--save_goal", "action": "store_true", "default": False,
            "help": "rather save goal image"},
        {"name": "--save_video", "action": "store_true", "default": False,
            "help": "rather excuted video"},
        {"name": "--agentago", "action": "store_true", "default": False,
            "help": "make video agent-agnostic"},
        #* END of PLANNING

        ##* START for TEST SAVING
        {"name": "--save_traj", "action": "store_true", "default": False,
            "help": "Run trained policy, no training, save the test result"},
        #* End of TEST SAVING

        ##* START for TRAIN setting
        {"name": "--panda", "action": "store_true", "default": False,},
        {"name": "--logtype", "type": str, "default": "Base",},
        {"name": "--plain_reward", "action": "store_true", "default": False,},
        #* END for TRAIN

        {"name": "--task", "type": str, "default": None, "required": True,
            "help": "Specific the task in the environment (args.env) to run"},
        {"name": "--camera", "type": str, "default": None, "required": False,
            "help": "Specific the camera sensor in the environment to run"},
        {"name": "--disable_wandb", "action": "store_true", "default": False,
            "help": "Disable wandb logging"},
        {"name": "--debug_vis", "action": "store_true", "default": False,
            "help": "Enable debug visualization"},
        {"name": "--randomize", "action": "store_true", "default": False,
            "help": "Apply env reset randomization"},
        ...
        {"name": "--cfg_repre", "type": str, "default": "Base"},
    ...
    logdir, cfg_train, cfg_env, cfg_repre = retrieve_cfg(args, use_rlg_config)
    ...
    if args.logdir == "logs/":
        args.logdir = logdir
        if args.logtype in ['ours', 'ours-onestage', 'ours-plainreward',
                            'clip', 'r3m', 'vip', 'liv', 'eureka']:
            logdir = args.logdir.split(os.path.sep)
            logdir.insert(1, args.logtype)
            args.logdir = os.path.sep.join(logdir)
        elif args.logtype == 'Base':
            pass
        else:
    ...
    if args.cfg_repre == "Base":
        args.cfg_repre = cfg_repre
